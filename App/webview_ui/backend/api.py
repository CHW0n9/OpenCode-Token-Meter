...
class JsApi:
    ...
    def get_stats(self, scope="today"):
        ...
        try:
            ...
            total_output = (stats.get("output", 0) or 0) + (
                stats.get("reasoning", 0) or 0
            )
            data = {
                ...
                for provider_id, models in provider_stats.items():
                    for model_id, model_stats in models.items():
                        cost = self.settings.calculate_cost(
                            model_stats, model_id, provider_id
                        )
                        providers.append(
                            {
                                "name": provider_id,
                                ...
                                "output": (model_stats.get("output", 0) or 0)
                                + (model_stats.get("reasoning", 0) or 0),
    ...
    def get_stats_by_model(self, scope="today"):
        ...
        try:
            ...
            for provider_id, models in data.items():
                for model_id, stats in models.items():
                    stats["cost"] = self.settings.calculate_cost(
                        stats, model_id, provider_id
                    )
            return self._format_response(True, data)
        except Exception as e:
    ...
    def get_stats_by_provider_range(self, start_ts, end_ts):
        ...
        print(
            f"[API] get_stats_by_provider_range called for range: {start_ts} to {end_ts}"
        )
        try:
            ...
            for provider_id, stats in data.items():
                ...
                if provider_id in model_breakdown:
                    for model_id, model_stats in model_breakdown[provider_id].items():
                        provider_cost += self.settings.calculate_cost(
                            model_stats, model_id, provider_id
                        )
        ...
        try:
            if system == "Darwin":
                ...
                try:
                    ...
                    self._log_debug(
                        "AppKit notification delivered successfully with delegate"
                    )
                except ImportError:
                ...
                try:
                    subprocess.run(
                        ["notify-send", "-a", "OpenCode Token Meter", title, message],
                        capture_output=True,
                    )
                    self._log_debug("Linux notify-send notification sent")
                except FileNotFoundError:
                    self._log_debug("notify-send not found")
                except Exception as e:
                    self._log_debug(f"Linux notification error: {e}")
    ...
    def get_thresholds_progress(self):
        ...
        try:
            ...
            today_cost = (
                self.settings.calculate_total_cost(today_models)
                if today_models
                else 0.0
            )
            month_cost = (
                self.settings.calculate_total_cost(month_models)
                if month_models
                else 0.0
            )
            ...
            monthly_token_thresh = self.settings.get(
                "thresholds.monthly_tokens", 10000000
            )
        ...
        try:
            ...
            if hasattr(self, "window") and self.window:
                ...
                out_path = os.path.join(
                    tempfile.gettempdir(), f"token_stats_{scope}.csv"
                )
            ...
            if mode == "model":
                        ...
                        rows.append(
                            {
                                ...
                                "output": (stats.get("output", 0) or 0)
                                + (stats.get("reasoning", 0) or 0),
                                "cost": cost,
                            }
                        )
                ...
                for provider_id, models in model_stats.items():
                    ...
                    for model_id, stats in models.items():
                        ...
                        total["output"] += (stats.get("output", 0) or 0) + (
                            stats.get("reasoning", 0) or 0
                        )
                        total["cost"] += self.settings.calculate_cost(
                            stats, model_id, provider_id
                        )
                    rows.append(total)
    ...
    def _build_trend(self, scope, timezone="local"):
        ...
        while current_ts <= end_ts and loop_guard < max_loops:
            ...
            s = bucket_stats.get(
                current_ts,
                {"input": 0, "output": 0, "reasoning": 0, "requests": 0, "cost": 0.0},
            )
    ...
    def _generate_distribution(self, providers, top_n=3):
        ...
        for p in providers:
            ...
            try:
                ...
                requests = 0
            items.append({"provider": provider, "model": model, "requests": requests})
