...
OLD_SETTINGS_PATH = os.path.join(
    os.path.expanduser("~"), "Library", "Application Support", APP_NAME, "settings.json"
)
...
class Settings:
    ...
    def __init__(self):
        ...
        if self._normalize_model_settings():
            self.save()
        self._sync_version_if_needed()

    def _sync_version_if_needed(self):
        """Compare settings/app version and persist app version when changed."""
        needs_update, _current_version, _app_version, _new_models, _customized_models = (
    ...
    def _smart_merge(self, base, override):
        ...
        if "prices" in result and isinstance(result["prices"], dict):
            ...
            if "default" in result["prices"] and isinstance(
                result["prices"]["default"], dict
            ):
    ...
    def calculate_cost(self, stats, model_id=None, provider_id=None):
        ...
        if not prices:
            # Hardcoded fallback
            prices = {"input": 0.5, "output": 3.0, "caching": 0.05, "request": 0.0}
        ...
        for provider_id, models in model_stats_dict.items():
            for model_id, stats in models.items():
                total_cost += self.calculate_cost(
                    stats, model_id=model_id, provider_id=provider_id
                )
        return total_cost
    ...
    def add_model_price(self, model_id, prices):
        ...
        default_models = DEFAULT_SETTINGS["prices"]["models"]
        if model_id in default_models and self._prices_match_default(
            prices, default_models[model_id]
        ):
    ...
    def delete_model_price(self, model_id):
        ...
        if (
            "models" in self.settings["prices"]
            and model_id in self.settings["prices"]["models"]
        ):
            del self.settings["prices"]["models"][model_id]
            self.save()
    ...
    def check_version_update(self):
        ...
        known_default_models = set(
            self.settings.get("prices", {}).get("known_default_models", [])
        )
    ...
    def update_version(self):
        ...
        self.settings["prices"]["known_default_models"] = list(
            DEFAULT_SETTINGS["prices"]["models"].keys()
        )
        self.save()
