        ...
        try:
            ...
            if count > 0:
                ...
                if current_interval != FAST_INTERVAL:
                    log_info(
                        "Agent",
                        f"Activity detected ({count} new). Switching to FAST mode ({FAST_INTERVAL}s)",
                    )
                current_interval = FAST_INTERVAL
                no_update_count = 0
                ...
                if (
                    current_interval == FAST_INTERVAL
                    and no_update_count >= TRANSITION_THRESHOLD
                ):
                    log_info(
                        "Agent",
                        f"No activity for {TRANSITION_THRESHOLD} scans. Switching to SLOW mode ({SLOW_INTERVAL}s)",
                    )
                    current_interval = SLOW_INTERVAL
                    no_update_count = (
                        0  # Reset counter (not strictly needed for slow mode but clean)
                    )
...
async def full_history_scan():
    ...
    try:
        ...
        if not bg_scanner.file_scan_enabled:
            return
        ...
        log_info(
            "Agent",
            f"Full history scan completed: {count} new/updated messages processed",
        )
    except Exception as e:
...
async def main(threading_stop_event=None):
    ...
    try:
        ...
        if not has_data:
            log_info(
                "Agent",
                "Fresh database detected. Performing FULL initial scan of all messages...",
            )
        ...
        if threading_stop_event:
            ...
            async def monitor_threading_event():
                ...
                log_info(
                    "Agent", "Threading stop event detected, shutting down agent..."
                )
                stop_event.set()
