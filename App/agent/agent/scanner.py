...
class Scanner:
                ...
                try:
                    ...
                    if "time" in data:
                        if isinstance(data["time"], dict):
                            ts = (
                                data["time"].get("created")
                                or data["time"].get("timestamp")
                                or 0
                            )
                        else:
                            ts = data["time"]
                    ...
                    if not role:
                        if (
                            tokens["input"] > 0
                            or tokens["output"] > 0
                            or tokens["reasoning"] > 0
                            or tokens["cache_read"] > 0
                            or tokens["cache_write"] > 0
                        ):
                            role = "assistant"
                        else:
                            role = "user"
    ...
    def scan_once(self, incremental=True, max_age_days=None, quick_start=False):
        ...
        if not os.path.isdir(MSG_ROOT):
            ...
            if (
                hasattr(self, "cache_days_loaded")
                and self.cache_days_loaded is not None
                        ...
                        try:
                            with os.scandir(ses_dir) as msg_it:
                                for msg_entry in msg_it:
                                    if (
                                        not msg_entry.is_file()
                                        or not msg_entry.name.endswith(".json")
                                        or not msg_entry.name.startswith("msg_")
                                    ):
                                        continue
                                    ...
                                    if (
                                        cutoff_time > 0
                                        and (file_mtime // 1_000_000_000) < cutoff_time
                                    ):
                                        continue
